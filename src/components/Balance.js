...
export const Balance = () => {
    ...
    const еxpenseAmounts = expenseTransactions.map(expenseTransaction => expenseTransaction.expenseAmount)

    const totalIncome = incomeAmounts.reduce((acc, item) => (acc += item), 0).toFixed(2)


    const totalExpenses = еxpenseAmounts.reduce((acc, item) => (acc += item), 0).toFixed(2)
    return (
